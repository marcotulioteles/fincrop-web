...
const MyFiles = () => {
  ...
  const [files, setFiles] = useState<{ fileName: string, fileSize: number, filePath: string }[]>([]);
  ...
  if (status === 'loading' || !session?.user) {
    return (
      <main className='w-full min-h-screen flex items-center justify-center bg-my-files'>
  ...
  return (
    <main className='w-full min-h-screen flex justify-center bg-my-files'>
      <div className='desktop:max-w-7xl sm:max-w-[95%] max-w-[90%] w-full flex flex-col items-start gap-4 mt-6'>
        ...
        <div className="w-full flex flex-col items-center justify-center rounded-3xl bg-white p-6 pb-20">
          <form className="w-full flex items-center justify-end gap-2 sm:flex-row flex-col">
            <input className="font-normal placeholder-gray-300 text-sm text-gray-900 px-3 py-2 border border-gray-200 flex-1 w-full sm:max-w-xs rounded" type="text" name="pesquisar" id="pesquisar" placeholder="Pesquisar em Arquivos RFP" />
            <button className="font-normal text-white text-xs p-3 bg-yellow-500 rounded w-full sm:w-fit">
              Pesquisar
            </button>
          </form>
