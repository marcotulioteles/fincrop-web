...
const Error = ({ error }: { error: Error }) => {
  return (
    <main className='w-full min-h-screen flex flex-col items-center justify-center bg-blue-800'>
      ...
      <div className="md:w-fit max-w-[90%] flex flex-col items-center justify-center gap-6 bg-white p-10 rounded-xl">
        ...
        <Link href='/' className='flex items-center justify-center gap-3 hover:scale-105 border border-gray-200 p-3 rounded-md'>
          <Undo2 className='w-5 h-5 text-gray-900'/>
          <span className='text-gray-900 text-base'>Voltar para Sign In</span>
          <Image src='/assets/icons/google-icon.svg' alt='Google Logo' width={24} height={24} />
        </Link>
      </div>
    </main>
  )
